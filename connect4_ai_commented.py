...
# for each position in the matrix the board is either filled with an empty WHITE circle, or a palyer/AI GREEN/DGREY circle
    ...
    for c in range(COLS):
        for r in range(ROWS):
            ...
            if board[r][c] == 0:
                ...
                pygame.draw.circle(screen, DGREY, (int(c * SQUARESIZE + SQUARESIZE/2), int(r* SQUARESIZE + SQUARESIZE + SQUARESIZE/2)), circle_radius)
...
while not game_over:
    ...
    if turn == AI_TURN and not game_over and not_over:
        ...
        if is_valid_location(board, col):
            ...
            if winning_move(board, AI_PIECE):
                print("PLAYER 2 WINS!")
                label = my_font.render("PLAYER 2 WINS!", 1, DGREY)
                screen.blit(label, (40, 10))
                not_over = False
                t = Timer(3.0, end_game)
                t.start()
